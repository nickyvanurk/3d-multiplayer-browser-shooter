...
export class AmmoPhysicsWorld implements PhysicsWorld {
  ready: boolean;
  ...
  shapes!: Record<number, Record<number, btCollisionShape>>;
  ...
  async init(): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
      ...
      loadingManager.onLoad = () => {
        this.shapes = {};
        this.shapes[Types.Entities.SPACESHIP] = {
          1: this.createShapeFromEntityType(Types.Entities.SPACESHIP),
        };
        this.shapes[Types.Entities.ASTEROID] = {
          1: this.createShapeFromEntityType(Types.Entities.ASTEROID),
        };
        this.shapes[Types.Entities.BULLET] = {
          1: this.createShapeFromEntityType(Types.Entities.BULLET),
        };
        resolve();
      };
  ...
  remove(entity: Entity): void {
    ...
    const body = entity.body as btRigidBody;
  ...
  createWorld(): btDiscreteDynamicsWorld {
  ...
  createRigidBodyConstructionInfo(
    entity: Entity,
  ...
  setupRigidBody(body: btRigidBody, entity: Entity): btRigidBody {
    const velocity = entity.velocity;
    const angularVelocity = entity.angularVelocity;
    ...
    if (entity.kinematic && body.setCollisionFlags && body.getCollisionFlags) {
      ...
      body.setCollisionFlags(
        body.getCollisionFlags() | CF_NO_CONTACT_RESPONSE | CF_KINEMATIC_OBJECT,
      );
  ...
  detectCollision(): void {
    ...
    const bulletsHit = new Set<Entity>();
    ...
    for (let i = 0; i < numManifolds; i++) {
      ...
      const entity0 = rb0.entity;
      const entity1 = rb1.entity;

      if (!entity0 && !entity1) {
        continue;
      }

      let kind0: EntityKind | undefined;
      let kind1: EntityKind | undefined;
  ...
  createShapeFromEntityType(type: EntityKind): btCollisionShape {
    if (type === Types.Entities.SPACESHIP) {
      return this.createConvexHullShape(
        this.assetManager.getTriangles('spaceship'),
      );
    }
    if (type === Types.Entities.ASTEROID) {
      return this.createConvexHullShape(
        this.assetManager.getTriangles('asteroid'),
      );
    }
    if (type === Types.Entities.BULLET) {
      return new this.ammo.btBoxShape(new this.ammo.btVector3(0.05, 0.05, 0.5));
    }
